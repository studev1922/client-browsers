...
const _sup = {
    ...
    clicker: async (...steps) => steps.forEach(async _ => await _sup._do.click(typeof st === 'string' ? _sup._do.click(st) : _sup._do.click(st.s || st.select, st.t || st.time))),
}; (async () => {
    const { log, _pr, _do, local } = _sup;
    // BEGIN CODE
