...
const _do = {
    ...
    async function sms_user() {
        ...
        // [10] GỬI TIN NHẮN
        await _do.click(_do.get(st.sms.input)?.lastChild, .5e3);

        console.info('SENT TO: ', name, at);
        return true;
    };

    async function mem_on_timeline(gr_t_loading = 1.5e3) { // chờ 1.5s để tải thành viên
        let gr_e, gr_mems;
    ...
    async function load_contact_sms() {
        ...
        await mem_on_timeline(1e3); // [2] CHỜ TẢI DS THÀNH VIÊN
    }
    ...
    , 'ánh dương fruit' // NHÓM CẦN TƯƠNG TÁC
